...
# Last modified: 2016-11-15 15:17
# Filename: manager.py
...
class TransferManager:
    ...
    def __init__(self, msg_port=9000, session_count=10, files_dir='', hook=None):
        ...
        # This is unreliable cause on some *nix systems will return 127.0.1.1
        # self._host_ip = socket.gethostbyname(socket.gethostname())
        # Use get_my_ip() instead.
        self._host_ip = self.get_my_ip()
    ...
    def _put(self, args, sender_host, msg_port):  # Put a file
        ...
        force = '-F' in args
        confirm_msg = u'来自用户:%s的%s' % (host, filename.rsplit('\\', 1)[-1])
        for func in self._confirms.get('_put', []):
            if func(confirm_msg) is not True:
                return self._rej_m(
                    mtype='PUT',
                    code='003',
                    reason='User reject')
    ...
    def _get(self, args, sender_host, msg_port):  # GET a file
        ...
        filename = args[args.index('-f')+1]
        force = '-F' in args
        confirm_msg = u'用户%s请求文件%s' % (host, filename)
    ...
    def handle_msg(self, msg, sender_host, sender_port):
        ...
        if (mtype != 'REG' and mtype != 'SRG') and \
            ...
            for hook in self._hooks:
                hook(old_msg, msg, sender_host, sender_port)
